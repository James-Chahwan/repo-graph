...
- "What's the request flow for creating a group?" → `flow` returns entry → service → data for that feature.
- "If I change this handler, what breaks?" → `impact` gives the blast radius by tier.
- "What's directly wired to this function?" → `neighbours` for the one-hop connections.
